...
def cps_convert_term(
    term: L2.Term,
    m: Callable[[L1.Identifier], L1.Statement],  # change to m?
    ...
    match term:
            ...
            return L1.Abstract(
                ...
                then=_term(
                    ...
                    lambda target: _terms(
                        arguments, lambda arguments: L1.Apply(target=target, arguments=[*arguments, k])
                    ),
                ),
            )
                ...
                then=_term(
                    ...
                    lambda left: _term(
                        ...
                        lambda right: L1.Branch(
                            ...
                            left=left,
                            right=right,
                            then=_term(consequent, m=lambda consequent: L1.Apply(target=k, arguments=[consequent])),
                            otherwise=_term(otherwise, m=lambda otherwise: L1.Apply(target=k, arguments=[otherwise])),
                        ),
                    ),
                ),
            )
        ...
        case L2.Load(base=base, index=index):  # base is a term so it is similar to primitive
            tmp = fresh("t")
            return _term(
                base,
    ...
    match program:
        case L2.Program(parameters=parameters, body=body):  # pragma: no branch
            return L1.Program(
                ...
                ),  # body=_term(body, lambda body: L1.Halt(value=body))
            )
